...
public class UserMomentsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Shares");
                            ...
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                ...
                                viewHolder.likes_but.setOnClickListener(new View.OnClickListener() {
                                    ...
                                    public void onClick(View v) {
                                        ...
                                        final Map updateoldmomentMap = new HashMap();
                                        updateoldmomentMap.put("by",By);
                                        updateoldmomentMap.put("timestamp",timestamp);

                                        if(liked.equals("false")){

                                            updateoldmomentMap.put("liked","true");
                                            postRef.child(Uid).child(moment_id).updateChildren(updateoldmomentMap).addOnSuccessListener(new OnSuccessListener<Void>() {
                                                @Override
                                                public void onSuccess(Void aVoid) {
                                            ...
                                            updateoldmomentMap.put("liked","false");
                                            postRef.child(Uid).child(moment_id).updateChildren(updateoldmomentMap).addOnSuccessListener(new OnSuccessListener<Void>() {
                                                @Override
                                                public void onSuccess(Void aVoid) {
                                                    allPostsRef.child(moment_id).child("likes").setValue(likes-1).addOnSuccessListener(new OnSuccessListener<Void>() {
